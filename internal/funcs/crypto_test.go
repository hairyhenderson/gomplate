...
func TestYescrypt(t *testing.T) {
	...
	t.Run("custom salt appears", func(t *testing.T) {
		t.Parallel()
		salt := "abc123"
		actual, err := c.Yescrypt(14, 8, salt, in)
		require.NoError(t, err)
	...
	t.Run("hash verifies", func(t *testing.T) {
		t.Parallel()
		hash, _ := c.Yescrypt(14, 8, "salt", in)
