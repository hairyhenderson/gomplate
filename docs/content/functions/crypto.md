...
```
crypto.Yescrypt [cost blockSize salt] input
```

```
input | crypto.Yescrypt [cost blockSize salt]
```
...
```console
$ gomplate -i '{{ "foo" | crypto.Yescrypt }}'
$y$jB5$YZZJoNING3pMhVIKFhIJJ/$n.9CRZ17bqvvDKyrAWqvghg7k5rq9M9F4rpWntnpeV0
```

```console
$ gomplate -i '{{ crypto.Yescrypt 10 1 "mysalt" "foo" }}'
